...
def test_agent_card_injects_deployment_context_into_extensions() -> None:
    ...
    assert streaming.params["stream_fields"]["idle"] == "metadata.shared.stream.idle"
    assert (
        streaming.params["stream_fields"]["since_last_chunk_ms"]
        == "metadata.shared.stream.since_last_chunk_ms"
    )
